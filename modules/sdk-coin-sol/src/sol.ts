...
export class Sol extends BaseCoin {
   ...
   */
  private verifyCloseAtaTransaction(transaction: Transaction, walletRootAddress: string): void {
    if (!walletRootAddress) {
      throw new Error('Cannot verify close ATA transaction: wallet root address is not available');
    }

    const txJson = transaction.toJson();

    for (const instruction of txJson.instructionsData) {
      if (instruction.type === 'CloseAssociatedTokenAccount') {
  ...
  async verifyTransaction(params: SolVerifyTransactionOptions): Promise<boolean> {
    ...
    if (verificationOptions?.verifyCloseAta) {
